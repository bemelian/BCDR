...
y_train = np.array([int(i[2]) for i in train])  # Метки букв (номер цифры + 65)
X_train = np.zeros((len(x), 20, 20))
X_train = x.reshape(len(x), 20, 20)

# ПОДГОТОВКА ТЕСТОВЫХ ДАННЫХ
test = open("ocrbtest.csv").read()
test = test.split("\n")[1:-1]
...
Y_train = np_utils.to_categorical(y_train-65, NB_CLASSES)
Y_test = np_utils.to_categorical(y_test-65, NB_CLASSES)
